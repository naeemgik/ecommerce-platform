...
public class ProductEventPublisher {
	...
	public CompletableFuture<Void> publishProductUpdated(Long productId, String sku, String name, String category) {
		...
		publishEvent(PRODUCT_CACHE_INVALIDATION_TOPIC, String.valueOf(productId),
				ProductEvent.ofCacheInvalidation(productId));
	...
	private CompletableFuture<Void> publishEvent(String topic, String key, ProductEvent event) {
		return kafkaTemplate.send(topic, key, event).thenAccept((SendResult<String, ProductEvent> result) -> {
			log.debug("Event published to topic '{}', partition: {}, offset: {}", topic,
